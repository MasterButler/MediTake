...
/**
 * Created by Winfred Villaluna on 4/12/2017.
 */

public class AddScheduleStepAdapter extends AbstractFragmentStepAdapter {

    ArrayList<Fragment> fragments;
    Context contextHolder;

    public AddScheduleStepAdapter(FragmentManager fm, Context context){
        super(fm, context);
    ...
    public StepViewModel getViewModel(@IntRange(from = 0L) int position) {
        ...
        String back = "BACK";
        if (position == 0) {
            title = AddScheduleDetailsFragment.TITLE;
            back = "DISCARD";
        }else if(position == 1){
            title = AddScheduleMedicineFragment.TITLE;
        }
